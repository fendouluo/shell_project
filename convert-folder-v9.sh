...
do
	...
	icnt=$[$icnt+1]
	result=$(ls "$convert_path" | grep "${folder_name}")
	echo "cmp is $result "	
	
	if [ -d "$convert_path/${folder_name}" ]; then
	...
	file_mediainfo=$path/${folder_name}.mediainfo
	file_audioinfo=$path/${folder_name}.audioinfo
	file_videoinfo=$path/${folder_name}.videoinfo
	file_streaminfo=$path/${folder_name}.streaminfo
	file_ffmpeg_convert_info=$path/${folder_name}.convertinfo
	file_ffmpeg_remove_info=$path/${folder_name}.removeinfo
	file_ffmpeg_remove_cmd=$path/${folder_name}.removecmdinfo
	file_mediaAVinfo=$path/${folder_name}.AVinfo
	file_convert_cmd=$path/${folder_name}.ffmpegcmdinfo
        file_ffmpeg_convert2_info=$path/$folder_name.bitrate.convertinfo
	...
	$ffmpeg_tool -y -threads $cpu_num -i  special123456.mkv -max_muxing_queue_size 1024 \
		-map 0 -c:a copy -c:s copy  \
		-b:v 5M -maxrate 10M ${folder_name}_abcdef.mkv < /dev/null 1>$file_ffmpeg_convert2_info 2>$file_ffmpeg_convert2_info
	rm special123456.mkv
	
	mkdir -p $convert_path/${folder_name}
	mv $file_ffmpeg_remove_cmd $file_streaminfo $file_ffmpeg_remove_info $file_ffmpeg_convert2_info \
	    $file_mediainfo  $file_audioinfo  $file_videoinfo  $file_ffmpeg_convert_info \
	    $file_mediaAVinfo $file_convert_cmd -t $convert_path/${folder_name}

	mv ${folder_name}_abcdef.mkv $convert_path/${folder_name}/${folder_name}.mkv
	cp *.qn* $convert_path/${folder_name}
	cp *.srt $convert_path/${folder_name}
	cp *.ass $convert_path/${folder_name}
	cd ${local_path}
	let index=icnt%4
	let indexcolor=i%8
